...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-black text-slate-800 uppercase tracking-tight mt-3">Editar Ciclo</h1>
        <p class="text-sm font-bold text-primary-light uppercase tracking-widest mt-1">{{ $cycle->name }}</p>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
        <form action="{{ route('admin.evaluation-cycles.update', $cycle) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $cycle->name) }}" class="w-full border border-slate-200 rounded-lg px-4 py-3 text-sm font-semibold text-slate-800 focus:border-accent focus:ring-0 outline-none @error('name') border-rose-300 @enderror">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ old('start_date', $cycle->start_date->format('Y-m-d')) }}" class="w-full border border-slate-200 rounded-lg px-4 py-3 text-sm font-semibold text-slate-800 focus:border-accent focus:ring-0 outline-none @error('start_date') border-rose-300 @enderror">
                    ...
                    <div>
                        ...
                        <input type="date" name="end_date" value="{{ old('end_date', $cycle->end_date->format('Y-m-d')) }}" class="w-full border border-slate-200 rounded-lg px-4 py-3 text-sm font-semibold text-slate-800 focus:border-accent focus:ring-0 outline-none @error('end_date') border-rose-300 @enderror">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" step="0.01" min="1" max="5" name="cutoff_score" value="{{ old('cutoff_score', $cycle->cutoff_score) }}" class="w-full border border-slate-200 rounded-lg px-4 py-3 text-sm font-bold font-mono text-slate-800 focus:border-accent focus:ring-0 outline-none @error('cutoff_score') border-rose-300 @enderror">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full border border-slate-200 rounded-lg px-4 py-3 text-sm font-semibold text-slate-800 focus:border-accent focus:ring-0 outline-none @error('status') border-rose-300 @enderror">
                            <option value="inactive" {{ old('status', $cycle->status) === 'inactive' ? 'selected' : '' }}>Inativo</option>
                            <option value="active" {{ old('status', $cycle->status) === 'active' ? 'selected' : '' }}>Ativo</option>
                            <option value="closed" {{ old('status', $cycle->status) === 'closed' ? 'selected' : '' }}>Fechado</option>
                        </select>
                ...
                <div class="border-t border-slate-200 pt-6">
                    ...
                    @php
                        $categories = [
                            ...
                            'avaliacao_usuario' => 'Aval. Usuário',
                        ];
                        $weights = old('weights', $cycle->weights ?? []);
                    @endphp
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        @foreach($categories as $key => $label)
                            <div class="bg-slate-50 p-4 rounded-xl border border-slate-200">
                                ...
                                <input type="number" name="weights[{{ $key }}]" value="{{ $weights[$key] ?? 1 }}" min="0" max="10" class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold font-mono text-slate-800 bg-white text-center focus:border-accent focus:ring-0 outline-none">
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="border-t border-slate-200 pt-6">
                    ...
                    <div class="overflow-x-auto bg-slate-50 rounded-xl border border-slate-200 p-4">
                        <table class="w-full text-left border-collapse" id="global-goals-table">
                            ...
                            @php
                                $globalGoals = old('global_goals', $cycle->global_goals ?? []);
                            @endphp
