...
<div class="max-w-7xl mx-auto space-y-6 pb-24">
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-xl font-bold text-slate-800 tracking-tight">Criar Perfil e Permissões</h1>
        </div>
                    ...
                    <div class="p-4 space-y-3 bg-white">
                        @if(count($modulePerms) > 0)
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" name="permissions[]" value="{{ $perm->name }}" class="w-4 h-4 rounded border-slate-300 {{ $isSuper ? 'text-blue-500 focus:ring-blue-500' : 'text-blue-500 focus:ring-blue-500' }} perm-checkbox" {{ (is_array(old('permissions')) && in_array($perm->name, old('permissions'))) ? 'checked' : '' }}>
