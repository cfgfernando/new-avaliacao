...
@section('title', 'Editar Secretaria / Lotação')
...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-black text-slate-800 uppercase tracking-tight">Editar Secretaria / Lotação</h1>
            <p class="text-sm font-bold text-slate-500 uppercase tracking-widest mt-1">Modifique os dados abaixo</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
        <form action="{{ route('admin.offices.update', $office) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $office->name) }}" required class="input-neo" placeholder="Ex: Secretaria Municipal de Obras Públicas">
            ...
            <div>
                ...
                <input type="text" name="sigla" value="{{ old('sigla', $office->sigla) }}" class="input-neo" placeholder="Ex: SMOP">
            ...
            <div class="pt-4 border-t border-slate-100">
                <label class="flex items-center gap-3 p-4 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
                    <input type="checkbox" name="is_active" value="1" class="w-5 h-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500" {{ old('is_active', $office->is_active ? '1' : '0') == '1' ? 'checked' : '' }}>
