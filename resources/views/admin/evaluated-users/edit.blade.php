...
@section('title', 'Editar Servidor Avaliado')
...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-black text-slate-800 uppercase tracking-tight">Editar Servidor Avaliado</h1>
            <p class="text-sm font-bold text-slate-500 uppercase tracking-widest mt-1">Atualize os dados cadastrais do servidor público</p>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
        <form action="{{ route('admin.evaluated-users.update', $evaluatedUser->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $evaluatedUser->name) }}" required class="input-neo" placeholder="Ex: João da Silva">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $evaluatedUser->email) }}" required class="input-neo" placeholder="Ex: joao.silva@prefeitura.gov.br">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="registration_number" value="{{ old('registration_number', $evaluatedUser->registration_number) }}" required class="input-neo" placeholder="Ex: 998877">
                ...
                <div>
                    ...
                    <input type="text" name="cargo" value="{{ old('cargo', $evaluatedUser->cargo) }}" required class="input-neo" placeholder="Ex: Assistente Administrativo">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="office_id" required class="input-neo">
                        ...
                        @foreach($offices as $office)
                            <option value="{{ $office->id }}" {{ old('office_id', $evaluatedUser->office_id) == $office->id ? 'selected' : '' }}>{{ $office->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="evaluation_group" required class="input-neo">
                        ...
                        <option value="geral" {{ old('evaluation_group', $evaluatedUser->evaluation_group) == 'geral' ? 'selected' : '' }}>Quadro Geral</option>
                        <option value="saude" {{ old('evaluation_group', $evaluatedUser->evaluation_group) == 'saude' ? 'selected' : '' }}>Saúde</option>
                        <option value="educacao" {{ old('evaluation_group', $evaluatedUser->evaluation_group) == 'educacao' ? 'selected' : '' }}>Educação</option>
                        <option value="guarda" {{ old('evaluation_group', $evaluatedUser->evaluation_group) == 'guarda' ? 'selected' : '' }}>Guarda Municipal</option>
                    </select>
            ...
            <div>
                ...
                <select name="evaluator_id" class="input-neo">
                    ...
                    @foreach($evaluators as $evaluator)
                        <option value="{{ $evaluator->id }}" {{ old('evaluator_id', $evaluatedUser->evaluator_id) == $evaluator->id ? 'selected' : '' }}>{{ $evaluator->name }} ({{ $evaluator->email }})</option>
                    @endforeach
                </select>
            ...
            <div class="pt-4 border-t border-slate-100">
                <label class="flex items-center gap-3 p-4 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
                    <input type="checkbox" name="has_active_pad" value="1" class="w-5 h-5 rounded border-slate-300 text-rose-600 focus:ring-rose-500" {{ old('has_active_pad', $evaluatedUser->has_active_pad ? '1' : '0') == '1' ? 'checked' : '' }}>
            ...
            <div class="pt-4 border-t border-slate-100 space-y-6">
                <div class="flex flex-col">
                    <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest font-mono">Alterar Senha de Acesso</h3>
                    <p class="text-xs text-slate-400 mt-1">Deixe os campos em branco se não desejar alterar a senha atual do servidor</p>
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest font-mono block mb-2 px-1">Nova Senha de Acesso</label>
                        <input type="password" name="password" class="input-neo" placeholder="Mínimo de 8 caracteres">
                        @error('password') <span class="text-xs text-rose-500 font-bold mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest font-mono block mb-2 px-1">Confirmar Nova Senha</label>
                        <input type="password" name="password_confirmation" class="input-neo" placeholder="Digite a senha novamente">
                    </div>
                </div>
            </div>

            <div class="pt-6">
                <button type="submit" class="btn-neo btn-primary w-full py-4 text-sm font-bold">SALVAR ALTERAÇÕES</button>
            </div>
        </form>
    </div>
</div>
